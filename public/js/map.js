...
const isPointInJeddah = (lat, lng) => {
  ...
  const isLatInRange = lat >= jeddahBounds.minLat && lat <= jeddahBounds.maxLat;
  const isLngInRange = lng >= jeddahBounds.minLng && lng <= jeddahBounds.maxLng;
...
export const mapModule = () => {
  ...
  proceedBtnEl.addEventListener('click', (e) => {
    ...
    if (!locationCoordinates)
      return Alert.displayFailure('Please select a location on the map!', 2000);
    console.log(locationCoordinates);
    // Validation to ensure the location is inside Jeddah
    if (!isPointInJeddah(locationCoordinates.lat, locationCoordinates.lng))
      return Alert.displayFailure(
        `Sorry we don't deliver outside Jeddah!`,
        2000,
      );
